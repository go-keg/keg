...
<html>
  ...
  <body>
    ...
    <script>
      const storagePrefix = window.location.pathname.split('/').filter(Boolean).join('_')+'_';
      const originalSetItem = localStorage.setItem;
      localStorage.setItem = function(key, value) {
        originalSetItem.call(this, storagePrefix + key, value);
      };
      const originalGetItem = localStorage.getItem;
      localStorage.getItem = function(key) {
        return originalGetItem.call(this, storagePrefix + key);
      };
